...
class Area extends MainModel{
    ...
    public function save(){
        $statement ="INSERT INTO conf_areas(id_genr_area,estado,id_empresa)
        values(:IdGenr_area,:Estado,:Id_empresa)";
        $sql = MainModel::getConection()->prepare($statement);
        $sql->bindParam(":IdGenr_area",$this->getId_area());
        $sql->bindParam(":Estado",$this->getEstado());
        $sql->bindParam(":Id_empresa",$this->getId_empresa());
        $sql->execute();
        return $sql->rowCount();
    }
}
